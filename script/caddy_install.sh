...
Download_caddy(){
	...
	if [[ ${bit} == "x86_64" ]]; then
		wget -O "caddy_linux.tar.gz" "https://caddyserver.com/download/linux/amd64${extension_all}"
	elif [[ ${bit} == "i386" || ${bit} == "i686" ]]; then
		wget -O "caddy_linux.tar.gz" "https://caddyserver.com/download/linux/386${extension_all}"
	elif [[ ${bit} == "armv7l" ]]; then
		wget -O "caddy_linux.tar.gz" "https://caddyserver.com/download/linux/arm7${extension_all}"
	else
...
Service_caddy(){
	if [[ ${release} = "centos" ]]; then
		if ! wget https://raw.githubusercontent.com/wym3253/ServerStatus-Toyo/master/script/caddy_centos -O /etc/init.d/caddy; then
		...
		if ! wget https://raw.githubusercontent.com/wym3253/ServerStatus-Toyo/master/script/caddy_debian -O /etc/init.d/caddy; then
			echo -e "${Error_font_prefix}[错误]${Font_suffix} Caddy服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/caddy
